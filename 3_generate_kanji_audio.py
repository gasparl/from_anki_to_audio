#!/usr/bin/env python3
"""
ANKI KANJI AUDIO-LEARNING GENERATOR V3
...
Designed for direct execution in Spyder:
  1. Keep this script beside the other V3 scripts. It reads
     ``kanji_audio_output_v3/translated_output_v3.json``.
...
AUDIOBOOK_BASENAME = "kanji_audio_learning_v3"
...
def update_unit_manifest(
    ...
    entry = {
        ...
        "target_reading": row.get("target_reading", ""),
...
def html_companion_text(volume_plan: Dict) -> str:
    ...
    for chapter in chapters:
        ...
        target_word = str(chapter.get("target_word", "")).strip()
        target_reading = str(chapter.get("target_reading", "")).strip()
        ...
        if target_word:
            ...
            if target_reading:
                target_text += f" ({target_reading})"
            source_text = f"Target: {target_text}; {source_text}"
        if supporting:
            source_text += "; supporting " + ", ".join(
                str(value) for value in supporting
            )
        search_text = (
            f"{number} {target_word} {target_reading} "
            f"{japanese} {english_text}"
        ).casefold()
        cards.append(
            "\n".join(
                [
...
def main() -> bool:
    print("=" * 72)
    print("ANKI KANJI AUDIO-LEARNING GENERATOR V3")
    print(
